  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 style={{ color: "var(--text-primary)", fontSize: 20, fontWeight: 800 }}>Knowledge Gaps</h1>
              ...
              <tbody>
                {gaps.map((g) => (
                  <tr key={g.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    ...
                    <td style={{ padding: "12px 16px", color: "var(--text-tertiary)", fontSize: 11 }}>{g.detected_at ? new Date(g.detected_at).toLocaleDateString() : "—"}</td>
                      ...
                      <div className="flex gap-1.5">
                        {g.status === "open" && (
                          <button disabled={acting === g.id} onClick={() => act(g.id, "acknowledge")}
                            ...
                            {acting === g.id ? <ArrowPathIcon className="animate-spin" style={{ width: 11, height: 11 }} /> : "Acknowledge"}
                          </button>
                        )}
                        {g.status !== "resolved" && (
                          <button disabled={acting === g.id} onClick={() => act(g.id, "resolve")}
                            ...
                            {acting === g.id ? <ArrowPathIcon className="animate-spin" style={{ width: 11, height: 11 }} /> : "Resolve"}
                          </button>
                        )}
                      </div>
                    </td>
